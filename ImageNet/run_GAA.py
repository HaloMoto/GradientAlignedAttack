...
# GAA
def GAA_Test(surrogate_model_list, victim_model, epsilon, alpha, bound, is_target, device, test_loader):
    # GAA
    steps=10
    decay=1.0
    loss_type='GAL'
    ...
    l2_list = []
    linf_list = []
    gaa = GAA(surrogate_model_list, eps=epsilon, alpha=alpha, steps=steps, decay=decay, loss_type=loss_type, norm=bound, victim_model=victim_model, max_queries=args.max_query)
    if is_target:
        gaa.set_mode_targeted_random()
    start_time = time.time()
    for data, target in test_loader:
        data, target = data.to(device), target.to(device)
        if is_target:
            target_labels = gaa._get_target_label(data, target)
            success, _, n_query, L2, Linf = gaa(data, target_labels)
        else:
            success, _, n_query, L2, Linf = gaa(data, target)
    ...
    success_count = (success_arr & (query_arr < (args.max_query + 1))).sum()
    total_count = len(success_arr)
    ASR = success_count / total_count * 100.
    avg_query = query_arr[success_arr & (query_arr < (args.max_query + 1))].mean()
    median_query = np.median(query_arr[success_arr & (query_arr < (args.max_query + 1))])
    avg_l2 = l2_arr[success_arr & (query_arr < (args.max_query + 1))].mean()
    avg_linf = linf_arr[success_arr & (query_arr < (args.max_query + 1))].mean()

    print(
        'GAA bound: {} with {} surrogate(s) is_target: {} epsilon: {} Test: ASR: {}/{} ({:.2f}%), avg_query: {}, median_query: {}, avg_l2: {}, avg_linf: {}'.format(
            bound, len(surrogate_model_list), is_target, epsilon, success_count, total_count, ASR, avg_query, median_query,
            avg_l2, avg_linf
        ))
    end_time = time.time()
...
def test_main():
    ...
    alpha=0.8/255
    infos_gaa = GAA_Test(model_list, victim_model, epsilon, alpha, bound, is_target, device, test_loader_1)
    np.save('saved_model/EnsSurro1_' + str(args.ensemble_surrogate1) + '_EnsSurro2_' + str(
        ...
        args.max_query) + 'epsilon_8_255_GAA_Linf_Untarget.npy', infos_gaa)
    ...
    np.save('saved_model/EnsSurro1_' + str(args.ensemble_surrogate1) + '_EnsSurro2_' + str(
        args.ensemble_surrogate2) +'_SingleSurro_'+args.model_type+ '_Victim_' + args.victim_model_type + '_maxQuery_' + str(
        args.max_query) + 'epsilon_8_255_GAA_Linf_Target.npy', infos_gaa)
    ...
    np.save('saved_model/EnsSurro1_' + str(args.ensemble_surrogate1) + '_EnsSurro2_' + str(
        args.ensemble_surrogate2) +'_SingleSurro_'+args.model_type+ '_Victim_' + args.victim_model_type + '_maxQuery_' + str(
        args.max_query) + 'epsilon_16_255_GAA_Linf_Target.npy', infos_gaa)
    ...
    np.save('saved_model/EnsSurro1_' + str(args.ensemble_surrogate1) + '_EnsSurro2_' + str(
        args.ensemble_surrogate2) +'_SingleSurro_'+args.model_type+ '_Victim_' + args.victim_model_type + '_maxQuery_' + str(
        args.max_query) + 'epsilon_12.27_GAA_L2_Untarget.npy', infos_gaa)
    ...
    np.save('saved_model/EnsSurro1_' + str(args.ensemble_surrogate1) + '_EnsSurro2_' + str(
        args.ensemble_surrogate2) +'_SingleSurro_'+args.model_type+ '_Victim_' + args.victim_model_type + '_maxQuery_' + str(
        args.max_query) + 'epsilon_12.27_GAA_L2_Target.npy', infos_gaa)
    ...
    np.save('saved_model/EnsSurro1_' + str(args.ensemble_surrogate1) + '_EnsSurro2_' + str(
        args.ensemble_surrogate2) +'_SingleSurro_'+args.model_type+ '_Victim_' + args.victim_model_type + '_maxQuery_' + str(
        args.max_query) + 'epsilon_24.54_GAA_L2_Target.npy', infos_gaa)
